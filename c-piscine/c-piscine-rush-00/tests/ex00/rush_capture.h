 ...
 *
 * Both halves of the rule are libc's on purpose. isprint() is exactly
 * 0x20..0x7e in the "C" locale, which is the one these harnesses run in (none
 * of them calls setlocale), and the hex form is printf's "%02x". Written out by
 * hand, this function would be a c-02 exercise's answer under another name, in
 * a file every clone of this repo ships (AGENTS.md §0).
 *
...
static inline char	*rc_escape(const char *b, size_t n)
{
	...
	for (i = 0; i < n; i++)
	{
		c = (unsigned char)b[i];
		if (c == '\\' || c == '\n')
			k += (size_t)snprintf(out + k, 3, "\\%c", c == '\n' ? 'n' : '\\');
		else if (isprint(c))
			out[k++] = (char)c;
		else
			k += (size_t)snprintf(out + k, 5, "\\x%02x", c);
	}
	out[k] = '\0';
	return (out);
}
