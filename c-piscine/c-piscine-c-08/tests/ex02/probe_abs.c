 ...
 *
 * What it catches: a macro that hands the work to libc's abs() leaves an
 * undefined `abs` symbol behind (`U abs` in nm), and c-08's subject says
 * "Allowed functions: None". Without this, that header passes every layer the
 * exercise has -- right output, clean Norm, correct file -- and is a flat KO at
 * the Moulinette.
 */
