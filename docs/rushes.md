...
Say which at the top of `c-piscine/c-piscine-rush-00/BUILD.bazel`. It ships with all five
...
`c-piscine/c-piscine-rush-01/BUILD.bazel`; it is deliberately not gated, because being
...
**42 issues its own `numbers.dict`.** The owner's working branch tracks two
copies of it (`c-piscine/c-piscine-rush-02/numbers.dict` and the one inside
`deliverable/ex00/`), because the module is turned in with one — they are 42's
file, not this repo's, and the template build strips every `*.dict`, so the
template branch carries none. On a fresh clone of the template, download yours and keep it wherever you
