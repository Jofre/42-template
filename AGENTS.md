...
- `c-piscine/c-piscine-rush-NN/deliverable/ex00/` — whatever files the team chose;
  ...
  for it.
- `c-piscine/c-piscine-bsq/deliverable/ex00/` — the map parser, the search, and the
...
from `//oracle` (§2). The template build runs `tools/answer_scan.sh`, which looks
for every answer in the zone anywhere outside it.
...
- One folder per **course**, one folder per **project** inside it, each named exactly
  ...
  `c-piscine-exam-prep/`, a study guide rather than a project), `c-piscine-reloaded/`
  (one project, `c-piscine-reloaded/`), and `cursus/` (the Common Core). A project
...
- Per-module layout:
  - `deliverable/` — **the only folder submitted to 42.** For C modules these are the
    ...
    **generated, disposable, and gitignored** — the answer is the generator
    ...
    the grader supplies a file (Reloaded's `ft_putchar.c`), the harness supplies
    it from `tests/`, and the student never writes or turns it in.
...
- **Bazel is the single entry point:**
  ...
  - `bazel run //c-piscine/c-piscine-shell-0N:generate` — rebuild a shell module's
    ...
    the same for Piscine Reloaded's generated exercises (ex00–ex05).
