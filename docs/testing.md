...
- **FIX FIRST** — a red in `norm`, `compile`, `files`, `forbidden` or
  ...
  header defining nothing cannot compile the subject's own `main`, and the ABS
  ones at `forbidden` as well — are never blamed on you.
...
`bazel test //c-piscine/c-piscine-c-10/... --local_test_jobs=1 --notest_keep_going`
...
```sh
bazel test //c-piscine/c-piscine-shell-01:ex04_output --test_tag_filters=manual --spawn_strategy=local
```
...
it and gitignored. Never hand-edit a shell deliverable (Reloaded's
`:generate` refuses to wipe one that holds files while its generator is still
the skeleton, since those files were never committed):

```sh
# 1. write the commands in generators/exNN.sh
bazel run //c-piscine/c-piscine-shell-00:generate     # 2. rebuild the deliverable
bazel test //c-piscine/c-piscine-shell-00:ex01_output # 3. check it
