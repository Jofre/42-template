...
/* These cases print the exact value ft_strcmp returns, not only its sign;
 * expected.txt pins it. The hi_* cases use bytes >= 0x80: whether a char is
 * read as signed or unsigned flips the sign of the answer for those bytes.
 * Which reading is right is in `man strcmp`. */
