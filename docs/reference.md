...
| `files` | basic | the deliverable holds exactly the files the subject names — including what a shell generator leaves behind |
...
| `valgrind` | strict | no leaks or invalid access. Leaks only where the subject allows `malloc`; the invalid-access and uninitialised-value half runs over `c_mem_check`'s probes too, where nothing allocates at all |
| `diff` | strict | a large seeded corpus, against the Rust reference in `//oracle`. The hand-written fixtures hold no byte above `0x7f`; these corpora seed `0x7f`/`0x80`/`0xff`, which is the whole signed-`char` class |
...
| `allocfail` | basic | one `malloc` refused at a time; the error must be reported, not dereferenced. Opt-in per exercise, and only where the subject or a man page it cites states the contract |
...
`compile` while still stubs. The ABS ones (c-08 ex02, Reloaded ex22) are red at
`forbidden` too: with no macro, `ABS(...)` is a call to a function nobody
authorised. `first_red.sh` asks `stub_check` before blaming either on you, which
...
repository, and its `deliverable/` is that repository's root.
...
c-piscine/                 <- the C Piscine
  ...
  c-piscine-exam-prep/     <- the final-exam study guide; not a project
...
A whole course at one level: `bazel test //c-piscine/... --test_tag_filters=lvl_basic,-manual`.
