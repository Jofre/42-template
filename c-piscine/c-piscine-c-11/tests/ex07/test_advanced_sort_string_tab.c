 ...
 * answer. A test file is not exempt from that: it ships, and it is greppable.
 * Nothing is lost by calling libc, because what this exercise checks is that
 * YOUR function uses the comparator it is handed, not what the comparator
 * does. The length comparator below uses libc's strlen for the same reason. */
