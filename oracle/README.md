...
`rush01.rs`, `rush01_bonus.rs`, `rush02.rs` and `bsq.rs` — and the rule check a
rush01 grid is judged by, in `rush01_validate.rs`. It has to be: the differential
test layer compares your output against it at runtime, so removing it would take
the harness down.

`shell00.rs` and `shell01.rs` hold the few references a shell check needs and
cannot build from a fixture — each one there so that a check does not have to
run the exercise's own command to know what to expect, which would put the
answer in a file the template ships:

- `oracle shell01_groups <user>` prints a user's groups in the order `id` gives
  them, from the machine's user database, through the same glibc calls `id`
  makes. shell-01 ex01's check compares against it.
- `oracle shell01_line_range <first> <last>` copies lines first..last of stdin
  to stdout. shell-01 ex07's check uses it to cut a window out of the
  student's OWN full output and compare it with their narrow-range output.
- `oracle shell01_hwaddr` reads lines on stdin and answers by exit code alone
  whether each is a MAC address and one of this machine's (read from
  `/sys/class/net`). shell-01 ex04's check asks it.
- `oracle shell00_diff_apply <old> <diff>` replays a normal-format diff onto a
  file and prints the result, refusing any diff that does not describe an edit
  of that file. shell-00 ex07's check builds its expected `b` with it from 42's
  `resources.tar.gz`.
...
it as "this exercise has no *differential* layer".
...
A few arms are plain loops over integers, and they stay that way on purpose: the
`#[inline(never)]` above each is what keeps it attributable under callgrind, and
rewriting them as iterator chains to widen the language gap would inline them
away and break the `cycles` layer.
...
(`c-piscine/c-piscine-rush-00/tests/ex00/rush_capture.h`). Escaping the backslash is not
...
### Which rule a grid breaks (rush01_validate)

Membership says *whether* a grid is right; a student needs to hear *why* it is
not — "row 2 holds the height 3 twice", "column 3 is seen as 2 from the top, but
clue 3 asks for 3". Working that out is the validity half of a rush01 solver, so
it lives here rather than in the runner: `src/rush01_validate.rs`, run as

```
oracle rush01_validate            # one request per stdin line, one reply each
request  <W> <H>\t<clues>\t<cells>
reply    <faults>\t<category>\t<message>\t<why>
```

`clues` are the 2W+2H values in the subject's order, `cells` the W*H values
row-major. `faults` counts every broken rule; `category` (`latin-row`,
`latin-col`, `clue`, or `ok` when nothing is broken) and the two sentences
describe the first one, in the order rows, columns, then clues as the input lists
them. The sentences are exactly the ones `tools/rush01_check.sh` prints — it used
to compute them itself, in awk that ported to C line for line. A malformed
request exits 2 after answering the lines before it. A cell is read as a
*number*, with awk's reading of it reproduced (see the module header), because
one of the runner's paths hands over a grid before every row is known to be
digits.
...
targets in `c-piscine/c-piscine-rush-01/BUILD.bazel`. This paragraph used to say the bonus
...
| `c06_sort_params` | c-06 ex03 | the same, in the order `man strcmp` defines — bytes above `0x7f` included |
...
corpus. Its `gen` output is `c-piscine/c-piscine-c-00/tests/ex08/expected.txt`, nine
...
`c-piscine/c-piscine-c-00/BUILD.bazel` already records at length why none of the five gets
...
and prints exactly the 598 bytes of `c-piscine/c-piscine-c-00/tests/ex05/expected.txt`, and
...
`rush01_bonus::check_rect()`, `rush01_validate::check()`, `bsq::check()`,
`shell01::check()` and `shell00::check()`, twenty-three calls in all — and a
non-zero total exits 1. (The two shell modules hold their line-range,
MAC-address and diff-replay references to hand-worked tables and to seeded
properties: a window cut three ways reassembles into its input; a diff built
from an LCS of two random files replays the first into the second byte for
byte, and every line it removes is verified against the old file.) (This list
named eleven and called the first rush
...
2. **Property assertions over a seeded sweep** — what must hold for *every*
   ...
   `atoi`/`putnbr` and `convert_base` round-trip; `strlcat`'s return agrees
   with the definition in `man strlcat` for every size, including a size below
   the length of `dest`; `strncmp` agrees in sign with `strcmp`;
   ...
   and infix equals sorted order; the `<shape>` field the c-13 harnesses link
   their trees from is well formed (one root, every item under an earlier one,
   no slot claimed twice), and ex04's probe routes find `build()`'s tree item
   for item and empty at every empty slot, and tell it apart from every tree
   built with a wrong rule, a lost insert or a doubled one; every board
   `c05_ten_queens` prints is legal
   ...
   *every* 1…60 × 1…60 box for all five variants; `c05_ten_queens` must agree
   ...
   `rush01_validate`, the rule checker the rush01 runner asks why a grid is
   wrong, is held to that same table: all 576 squares against all 438 vectors,
   where "no rule broken" must coincide exactly with "listed" (576 passing pairs
   of 252 288). On top of it, 2 000 seeded made-up boards of every shape
   1..9 x 1..9 — a shuffled cyclic Latin square cut to size, never solved for —
   must pass against clues counted by a separate re-scanning counter, and each
   mutation of one (a repeat in a row, three of a kind, a swap that repeats a
   height in a column, a wrong clue on every slot of all four sides) must be
   rejected with the category and the sentence the mutation predicts. Its
   hand-written table pins the exact reply bytes, awk's reading of odd cell
   values included.
...
just do not gate the reference. (Nor is a fixture ever computed from C. It is
typed from the subject or taken from `//oracle`'s own output, never produced by
running a harness against a hand-written C reference: that reference would be
the exercise's answer in a file outside the zone, which AGENTS.md §0 rules out.
`tools/gen_expected.sh`, the helper that offered that workflow, was deleted for
it.)
...
`c-piscine/c-piscine-c-05/tests/ex08/expected.txt` are the same 7 968 bytes (md5
...
| --- | --- | --- | --- |
| `c00_print_alphabet` | `c-piscine/c-piscine-c-00/tests/ex01/expected.txt` | 26 | `c3fcd3d7…` |
...
`c_diff(num = "03")` in `c-piscine/c-piscine-c-04/BUILD.bazel`.
...
cargo run --offline -- c05_ten_queens 0 0 | cmp - ../c-piscine/c-piscine-c-05/tests/ex08/expected.txt
cargo run --offline -- c00_print_alphabet 0 0 | cmp - ../c-piscine/c-piscine-c-00/tests/ex01/expected.txt
cargo run --offline -- c00_print_reverse_alphabet 0 0 | cmp - ../c-piscine/c-piscine-c-00/tests/ex02/expected.txt
cargo run --offline -- c00_print_numbers 0 0 | cmp - ../c-piscine/c-piscine-c-00/tests/ex03/expected.txt
cargo run --offline -- c00_print_comb 0 0 | cmp - ../c-piscine/c-piscine-c-00/tests/ex05/expected.txt
cargo run --offline -- c00_print_comb2 0 0 | cmp - ../c-piscine/c-piscine-c-00/tests/ex06/expected.txt
...
cargo run --offline -- bench c00_print_comb2 </dev/null | cmp - ../c-piscine/c-piscine-c-00/tests/ex06/expected.txt
