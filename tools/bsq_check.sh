...
# with the reference. 128+N is "killed by signal N", and report_case already
# took $rc for exactly this purpose -- it was simply never consulted on the
# path where the bytes matched.
#
...
if [ "${NO_SKIP:-0}" != "1" ] &&
	...
	sh "$GATE_DIFFER" --bin "$GATE_BIN" --expected "$GATE_EXPECTED" > /dev/null 2>&1
	_gate_rc=$?
	if [ "$_gate_rc" -ge 2 ]; then
		echo "bsq_check: the output gate could not be evaluated (exit $_gate_rc),"
...
while IFS= read -r line; do
	...
	if [ "$MODE" = multi ]; then
		...
		run_one "$GARGS"
		rc=$?
		if ! cmp -s "$WORK/want" "$WORK/got" || [ "$rc" -ge 128 ]; then
			BAD=$((BAD + 1))
