#!/bin/sh
# ex00 — exo.tar must hold the 7 items test0..test6 with the exact member
# TYPE + permission string and the two link relationships shown by ls -l in the
# subject, the exact content SIZE of the regular files, and each item's date and
# time. Only what the subject prints as "XX" is ignored: the owner, the group,
# and the directories' sizes.
...
ck "exo.tar exists" test -f "$TAR"
ck "exo.tar is a readable tar archive" tar -tf "$TAR"
...
# year will be accepted instead of the time", which ck_listed_at applies. tar
# keeps each member's date and time, and -tvf prints them in full.
stampof() { printf '%s\n' "$tv" | grep -E "$1" | awk '{print $4, $5; exit}'; }
ck_listed_at "test0" "$(stampof ' test0/$')" 06-01 20:47
ck_listed_at "test1" "$(stampof ' test1$')" 06-01 21:46
ck_listed_at "test2" "$(stampof ' test2/$')" 06-01 22:45
ck_listed_at "test3" "$(stampof ' test3$')" 06-01 23:44
ck_listed_at "test4" "$(stampof ' test4$')" 06-01 23:43
ck_listed_at "test5" "$(stampof ' test5 link to test3$')" 06-01 23:44
ck_listed_at "test6" "$(stampof ' test6 -> test0$')" 06-01 22:20
