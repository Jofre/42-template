...
#               against files the grader supplies: Piscine Reloaded ex24
#               ("We'll only fetch your Makefile and test it with our files").
#               The harness's own copies are placeholders that compile, never
#               an exercise's answer. A DEST the student's tree already holds is
#               a FAIL: the grader fetches the Makefile alone, so that file is an
#               extra one in the turn-in, and the files layer cannot see it when
#               it sits in a subdirectory.
...
require basename cp dirname find grep head mkdir mktemp rm sed sleep sort tr
...
while [ $# -gt 0 ]; do
	case "$1" in
		...
		--overlay)
			...
			case "$3" in
				/* | ../* | */../* | "") echo "make_test.sh: --overlay DEST must be a relative path inside the exercise: '$3'" >&2; exit 2 ;;
			esac
...
# grader fetches the Makefile alone, so it is an extra file in the turn-in --
# and //tools:submit would push it -- while the files layer, which only lists
# the exercise directory itself, cannot see one under srcs/ or includes/. It is
# also what anyone does first to try their Makefile by hand, which is why the
# message says where to do that instead.
#
# A here-document, not a pipe, so the loop runs in this shell and its exit is
# this script's.
if [ -n "${OVERLAYS:-}" ]; then
	while IFS= read -r _o; do
		_src=${_o%%=*}
		_dst=${_o#*=}
		if [ -e "$WORK/$_dst" ] || [ -L "$WORK/$_dst" ]; then
			{
				echo "make_test.sh: FAIL — your turn-in holds '$_dst', which the grader supplies."
				echo "  The subject fetches your Makefile and nothing else, and builds it"
				echo "  against its own files, so this one is an extra file in what you"
				echo "  push. Remove it from the exercise directory. To try the Makefile"
				echo "  by hand, copy the grader's files and your Makefile somewhere"
				echo "  OUTSIDE deliverable/ and run make there (the README shows how)."
			} >&2
			exit 1
		fi
...
# recipe that recompiles and re-archives everything on every run is
...
'
		...
		if [ -n "$REDONE" ]; then
			{
				...
				if [ "$GRAPH" = 1 ]; then
					...
					echo "  Makefile you turn in, does: 'If the makefile relinks when not"
					echo "  necessary, the project will be considered non-functional.'"
	...
	if has_rule all; then
	rm -f "$ARTIFACT"
	LOG=${LOG:-$(mktemp)}
	if ! "$MAKE_BIN" all > "$LOG" 2>&1 || [ ! -f "$ARTIFACT" ]; then
		{
			...
			echo "  produce '$ARTIFACT' on its own."
