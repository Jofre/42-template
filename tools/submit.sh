...
#     strict    + what is real but not certain: the evaluator's main(), memory,
#               and a differential against the reference
#     robust    + this repo's own sanitizer rigour
...
require cat chmod dirname git grep head mkdir mktemp rm sed sort tail tr
...
#            allocfail  a refused malloc whose answer the code ignores. Opt-in
#                       per exercise and enabled at two, both of which cite the
#                       contract: c-08 ex04's subject states the NULL return in
#                       its own words, and c-07 ex00's says "reproduce the
#                       behavior of strdup (man strdup)", where the man page
#                       does. Where no such sentence exists the layer is not
#                       emitted at all.
#
...
#            diff       a seeded corpus against the Rust reference, over inputs
#                       nobody types. It is here rather than in basic because a
#                       disagreement can be a case the subject left open --
#                       and here rather than above because the hand-written
#                       fixtures carry no byte above 0x7f while these corpora
#                       seed 0x7f/0x80/0xff, so the signed-char class was
#                       invisible to every gate below it
#            valgrind   a block allocated and never accounted for. No subject
#                       in this Piscine mentions leaks, valgrind or memory
#                       management at all -- all twenty en.subject.pdf files
#                       were extracted and grepped, zero hits -- so whether 42
#                       looks is unknown, and this line used to assert that it
#                       does. Real, not certain: exactly what `strict` is for.
#
# robust = this repo's own rigour, beyond any sentence in a subject:
#            diff_asan  the differential corpus replayed under ASan/UBSan, which
#                       compares no values -- it reports sanitizer findings, and
#                       no 42 grader runs a sanitizer
#            asan       adversarial memory probes
#
...
if ! gate_tags "$GATE_LEVEL" >/dev/null 2>&1; then
	...
	echo "                       prototype, allocfail — what the Moulinette" >&2
	echo "                       grades (default)" >&2
	echo "             strict    + symbols, valgrind, diff" >&2
	echo "             robust    + diff_asan, asan" >&2
...
# rejected the key (the host key is fixed here; the student's own key is shown
# to them for registering, never created); conn = the server was unreachable (EXPECTED
...
run_gate() {
	...
	GATE_ERR=$(mktemp)
	# --max_idle_secs, and a shutdown after. This server exists ONLY for the
	# duration of a submit -- nothing else ever runs a command against
	# $GATE_BASE -- so left alone it is a JVM holding several hundred MB on a
	# machine that has none to spare, for Bazel's default three-hour idle
	# timeout, after a command the student ran once. A campus box is 8 GB.
	#
	# The output BASE stays on disk on purpose: it is what makes the second
	# submit fast. It is the SERVER that has no reason to outlive the gate.
	bazel --output_base="$GATE_BASE" --max_idle_secs=300 test "//$1/..." \
		--test_tag_filters="$(gate_tags "$GATE_LEVEL")" --keep_going \
		>/dev/null 2>"$GATE_ERR"
	_gate_rc=$?
	bazel --output_base="$GATE_BASE" shutdown > /dev/null 2>&1 || true
	return $_gate_rc
}
