...
#
# Usage:
#   shell_test.sh --generator PATH --mode norm|diff|check|files [options] [-- ARGS...]
#
...
require basename cat cp grep mktemp rm sed
...
$(abspath "$2")"; shift 2 ;;
		...
		--required) need "$1" "$#"; FREQUIRED="$FREQUIRED $2"; shift 2 ;;
		--optional) need "$1" "$#"; FOPTIONAL="$FOPTIONAL $2"; shift 2 ;;
...
[ "$MODE" = diff ] || [ "$MODE" = check ] || [ "$MODE" = files ] || {
	echo "shell_test.sh: --mode must be norm|diff|check|files" >&2; exit 2; }
...
# in deliverable/exNN and //tools:submit pushes whatever is there. So the set of
# names here is exactly the set 42 would receive.
#
