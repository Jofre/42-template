 ...
 *   - src : src need not be NUL-terminated when it holds n or more bytes, so the
 *           byte at index n may not exist. The last case passes a block of
 *           EXACTLY n bytes with no terminator: reading src[n] is out of bounds.
 * A correct strncpy is clean on all cases, incl. sizes 1 and 0. This is a test
 * input, not an implementation. */
#include <stdlib.h>
