...
# lives in //oracle now, in Rust (oracle/src/rush01_validate.rs: AGENTS.md §2
# makes the oracle a door a student may read and cannot paste), and this runner
# ASKS it, as `oracle rush01_validate`, one request per grid:
#
...
if [ -n "$GATE_DIFFER" ] && [ -n "$GATE_BIN" ] && [ -n "$GATE_EXPECTED" ] &&
	...
	# The differ has TWO meanings for non-zero: 1 is "the student's fixture is
	# red", which this gate exists for, and 2 is "the differ could not run at
	# all". `GATE_OK` collapsed them, so a BROKEN gate silenced the layer and
	# printed a false sentence while doing it. Skip on 1; on 2-or-more say so
	# and run anyway.
	if [ "$GATE_RC" -ge 2 ]; then
		echo "rush01_check: the output gate could not be evaluated (exit $GATE_RC),"
...
#               "<n>\t<nsol>\t<sols>\t<clue>" and apply --max-size / --max-cases.
...
RUSH01_ORACLE="$ORACLE" RUSH01_REQ="$T/req" awk -f "$T/rush01.awk" -v mode=prep \
	-v maxsize="$MAXSIZE" -v maxcases="$MAXCASES" -v casefile="$T/cases" \
...
judge_one() {
	...
	RUSH01_ORACLE="$ORACLE" RUSH01_REQ="$T/req" awk -f "$T/rush01.awk" -v mode=judge \
