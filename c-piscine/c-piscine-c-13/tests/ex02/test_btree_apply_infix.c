...
int	main(void)
{
	...
	r[1].right = &r[2];
	printf("-- null --\n");
	btree_apply_infix(NULL, print_item);
	printf("-- single --\n");
	btree_apply_infix(&single, print_item);
	printf("-- balanced --\n");
	btree_apply_infix(&b[3], print_item);
	printf("-- left chain --\n");
	btree_apply_infix(&l[2], print_item);
	printf("-- right chain --\n");
	btree_apply_infix(&r[0], print_item);
	return (0);
}
