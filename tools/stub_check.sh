...
# //...` down on a fresh clone. c_libft now compiles that program at test time
# inside tools/header_check.sh (--shape libft), so the same stub is one red
# test with the compiler's diagnostics under it, and ft.h is held to the
# guards-only rule like every other header. An approval is never the fix for a
...
35693185426d45ffe2dc66cae797d277 c-piscine/c-piscine-rush-00/deliverable/ex00/rush.h reduced-and-reviewed
'
...
if [ "$MODE" = file ]; then
	...
	# *.h. The same hole was open one kind further along:
	# c-piscine/c-piscine-rush-02/deliverable/ex00/README.md was a group planning document
	...
	# *.h so it was never re-stamped, the template build excludes deliverable/
	# from its staged identity grep, and --file mode exits 2 on such a file -- so
	# nothing in the publish path looked at it at all. (The file is gone now,
	# with the rest of that team's sources; the rule it taught stays.)
	#
	...
	# The same rule for generators/. AGENTS.md §0 puts EVERYTHING under a
	# generators/ in the zone, and only generators/exNN.sh is read above, so a
	# helper script or a data file a generator copies out would otherwise ship
	# under an OK verdict with nothing having opened it.
	gen_other=$(git ls-files "$_prefix/generators/*" 2>/dev/null |
		grep -vE '/generators/ex[^/]*\.sh$' || true)
	...
	# project: its BUILD file and README, its subject (or the template's
	# placeholder), 42's issued resources (which the template build strips), and
	# the exam guide, which is a study document rather than a project.
	if [ -z "$scope" ]; then
		course_files=$(git ls-files 'c-piscine/*' 'c-piscine-reloaded/*' 'cursus/*' 2>/dev/null)
	else
		course_files=$(git ls-files "$_prefix/*" 2>/dev/null)
	fi
	outside_files=$(printf '%s\n' "$course_files" | grep -v '^$' |
		grep -vE '/(deliverable|generators|tests)/' |
		grep -vE '(^|/)(BUILD\.bazel|README\.md|\.gitignore)$' |
		grep -vE '\.pdf$|groups_subject\.txt$|subject-ammendment\.md$|resources\.tar\.gz$|\.dict$' |
		grep -vE '(^|/)c-piscine-exam-prep/' || true)
fi
