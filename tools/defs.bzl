...
Exercise shapes:
  ...
  c_libft     -> c-09's shape: loose sources built into an archive by a build
                 ...
                 Its output layer compiles inside the runner too, so a header
                 turned in beside the sources can be a stub without breaking
                 the build.
...
# so this no longer reaches /usr/bin/ld.bfd through PATH (TODO.md §21, #3).
...
_LAYER_LEVEL = {
    ...
    # byte >= 0x80, and ONLY exNN_diff sees it. The hand-written fixtures carry
    # no byte above 0x7f; the generated corpora seed 0x7f/0x80/0xff on purpose.
    # So the layer holding the interesting bytes was the one held above the gate.
    #
...
def lvl_tags(layer, level = None):
    """The tags a HAND-WRITTEN sh_test needs: its layer, plus its level ladder.

    _test() derives these for every test a macro emits, but a handful are
    written out by hand -- the corpus runners no macro emits (bsq's three,
    c-06's argv differentials, c-10's file differentials, rush-01's sweep and
    its guard) and the suite's own selftests, which live outside every module.
    Each of those spelled the cumulative lvl_* pair literally, thirteen sites
    in six files, most carrying a comment restating that `diff` is level 3.

    A level written down in fourteen places is not a level; it is thirteen
    chances to disagree with _LAYER_LEVEL. Seven of the thirteen are `diff`,
    so moving that layer meant finding and hand-editing seven tag lists spread
    across four modules -- and a missed one does not fail, it silently keeps
    the old gate.

    Args:
        layer: the layer tag, as a string -- "diff", "selftest", "oracle".
            fail()s on anything _LAYER_LEVEL does not know, because the failure
            mode of a typo here is silence: _level_tags returns NO tag for an
            unrecognised tag, so the test would drop out of every level suite
            and every gate while still passing happily on its own.
        level: raise THIS test above where its layer sits, exactly as _test's
            `level` does, and refused downward for exactly the same reason --
            `basic` has to keep meaning everything the Moulinette would KO.

    Returns:
        The layer tag followed by its cumulative lvl_* tags, ready to hand to
        sh_test's `tags`. Anything else a target needs -- the `manual` on
        rush-01's bonus -- concatenates onto it, and does not affect the level:
        only layer tags do.
    """
    if layer not in _LAYER_LEVEL:
        fail(("lvl_tags: %r is not a layer. Known layers: %s. An unrecognised " +
              "tag gets no lvl_* tag at all, which drops the test out of every " +
              "level suite silently -- so this refuses rather than emits it.") %
             (layer, ", ".join(sorted(_LAYER_LEVEL))))
...
def c_levels(name = None):
    ...
    Args:
      name: unused. Bazel convention is that every macro takes a `name`, and
        ...
        bazel test //c-piscine/c-piscine-c-05:basic       # what the Moulinette grades
        bazel test //c-piscine/c-piscine-c-05:strict      # + symbols, valgrind
        bazel test //c-piscine/c-piscine-c-05:robust      # + diff, diff_asan, asan
        bazel test //c-piscine/c-piscine-c-05/...         # everything, as before
    ...
    # (//c-piscine/c-piscine-c-05:ex00_output) or a whole module, and nothing in between:
...
def _forbidden_test(name, srcs, allowed, hdrs = None, extra_tags = None, provided = None):
...
def _prototype_test(ex, srcs, hdrs = None, extra_includes = None, required = True):
    ...
    p_args = _pinned_cc_args() + ["--proto", "$(location %s)" % proto_f]
    for s in srcs:
...
def c_function(
    ...
    Args:
        ...
        provided_srcs: sources the GRADER compiles in with the student's, from
            tests/ -- Piscine Reloaded's ft_putchar.c: its subject says "If
            ft_putchar() is an authorized function, we will compile your code
            with our ft_putchar.c", and a student who turns one in has left an
            extra file. They join every BUILD of the student's code (both
            libraries, so the harness, c_diff and c_perf get them; ilp32;
            allocfail) and none of the layers that judge the student's own
            files: norm, compile, forbidden, prototype and symbols see `srcs`
            ...
            inside it. And each provided source defines its functions WEAK:
            a student's second definition then wins the link instead of
            breaking the build for every other test (see tests/ft_putchar.c).
    ...
    provided_srcs = provided_srcs or []
    deps = deps or []
    ...
    if exports == None:
        exports = [fn]
    if exports:
        sym_args = _pinned_nm_args() + ["--expect", ",".join(exports)]
        for p in _provided_names(provided_srcs):
            sym_args += ["--provided", p]
        for s in srcs:
        ...
        _test(
            ...
            srcs = ["//tools:symbols_test.sh"],
            args = sym_args,
            data = srcs + all_hdrs + _PINNED_NM_DATA,
            tags = ["symbols"],
        )
...
def c_mem_check(num, fn, probe, srcs = None, hdrs = None, provided_srcs = None):
    ...
    Args:
        ...
        hdrs: headers the probe or the sources include but which do not live
            ...
            the c_function() call. Their directories reach the -I list of both
            the sanitized compile and the plain one.
        provided_srcs: sources the grader compiles in with the student's (see
            c_function); both probe builds link them.
    """
    ex = "ex" + num
    hdrs = hdrs or []
    if srcs == None:
        srcs = _deliverable_srcs(ex, fn)
    srcs = srcs + (provided_srcs or [])
    ...
    cc_binary(
        # hdrs go in `srcs`: cc_binary has no hdrs attribute, and a header that
        # is not an input is not staged into the sandbox at all -- the compile
        # then dies on "file not found" for a file sitting in the source tree,
        # which reads as a broken deliverable and is a broken BUILD file.
        name = ex + "_memprobe",
        srcs = [probe_f] + srcs + hdrs,
        includes = probe_dirs,
        copts = _COPTS,
        tags = ["manual"],
    )
...
def c_program(
    ...
    Args:
        ...
        provided_srcs: sources the grader compiles in with the student's (see
            c_function): both program binaries link them; norm, compile and
            forbidden see the student's `srcs` alone. Not with `makefile`: a
            Makefile exercise builds what its Makefile names.
    """
    ex = "ex" + num
    ...
    if makefile and provided_srcs:
        fail("c_program(%s): provided_srcs has no effect with makefile -- " % ex +
             "the Makefile decides what is compiled.")
...
def c_files(num, required, optional = None, strict = False, subdir = None, label = None):
    ...
    Args:
        ...
        label: the heading the report is printed under; default None builds
            "<package>/<dir>", e.g. c-piscine/c-piscine-c-00/deliverable/ex00. fail()s if
...
def c_cycles(
    ...
    Args:
        ...
        syscall_budget: write() calls per case, counted from callgrind's own
            ...
            per case -- see c-02 ex11's call site, where a 0 copied from the
        ...
        hdrs: headers the harness and the sources include. Default None globs
            deliverable/exNN/*.h -- c-12's ft_list.h, c-13's ft_btree.h. A
            header nobody declares is not staged into the sandbox at all, so
            before this existed every c-12/c-13 cycles target died at its -O2
            rebuild on "'ft_list.h' file not found" the moment NO_SKIP=1
            opened its gate: a broken BUILD rule wearing the look of a broken
            deliverable.
    ...
    if hdrs == None:
        hdrs = native.glob(["deliverable/%s/*.h" % ex], allow_empty = True)
    for h in hdrs:
        args += ["--inc", "$(location %s)" % h]
    if unit_expr:
...
def c_libft(
        ...
        prototype = True,
        labeled = False):
    """A libft bundle (c-09): an archive the student's own build system produces.
    ...
    Args:
        ...
        srcs: the loose sources the student turns in, listed explicitly rather
            ...
            srcs/ subdirectory. They are compiled DIRECTLY together with `test`
            -- not taken out of the archive, which is exNN_build's business --
            so the behaviour layers keep working while the student's build
            system does not.
        test: a main() under tests/exNN/ exercising all of `srcs`. exNN_output
            ...
            body for why that is not a cc_binary. The same sources also build
            :exNN_bin, a cc_binary tagged manual, for the correctness gates:
            ...
            file name is yours (c-09 spells it test_libft.c in both exercises).
        ...
        hdrs: headers turned in beside the sources -- c-09 ex01's
            ...
            compiled into the output layer's test program, and their directory
            joins the include path of the compile, forbidden, prototype and
            symbols layers, so a header that breaks the Norm or contradicts a
            signature is caught like any other deliverable -- and one that is
            still a stub is a red exNN_output, not a build error.
        includes: extra include directories, package-relative, put on the
            output layer's compile, the cc_binary and the prototype layer.
            c-09 ex01 needs "deliverable/ex01/includes" so that a source in
            srcs/ can #include "ft.h" from its sibling directory.
    ...
    # THE OUTPUT LAYER COMPILES AT TEST TIME, not through a cc_binary -- the
    # same move, for the same reason, as c_header's (see its body).
    #
    # The harness main() reaches the bundled functions through whatever the
    # student turned in, and for c-09 ex01 that includes a HEADER. A header
    ...
    # built from it is a BUILD failure: `bazel test //...` on a fresh clone
    ...
    # it -- and no gate is attached to an exercise that turns in a header.
    # Attach one to such an exercise and the build error comes back with it.
    binname = ex + "_bin"
    test_f = "tests/%s/%s" % (ex, test)
    cc_binary(
        name = binname,
...
def c_make(num, anchor, artifact, symbols = None, script = None, rules = None, graph = False, relink = False, relink_source = None, overlay = None):
    ...
    Args:
        num: the exercise number as a two-character STRING, "00", "05". It
            names the target (exNN_build) and the deliverable directory whose
            contents become the test's inputs, so "0" would look in
            deliverable/ex0/ and emit a target nobody can find.
        anchor: a label inside the directory make is run in, conventionally
            deliverable/exNN/Makefile. The runner takes its DIRECTORY, which is
            how the build gets a working directory without one being spelled
            out a second time.
        ...
        rules: which make rules THIS subject mandates, e.g. ["clean", "fclean"].
            ...
            and c-09 has no c_make call at all. A docstring that keeps a census
            of its own callers is a docstring that goes stale -- state the
            rule, not the roll call.)
        ...
        relink_source: where the no-relink rule comes from, for the failure
            ...
            project whose subject never says it and whose Makefiles fall under
            the Norm's "If the makefile relinks when not necessary, the project
            will be considered non-functional" (Piscine Reloaded).
        overlay: files the GRADER supplies, staged beside the student's
            Makefile before the build, as {package-relative source: destination
            relative to the exercise directory}. For a turn-in that is a Makefile
            alone -- Piscine Reloaded ex24 says "We'll only fetch your Makefile
            and test it with our files". Keep the harness's copies placeholders
            that compile, never an exercise's answer. Default None.
    """
    ex = "ex" + num
    symbols = symbols or []
    ...
    _test(
        ...
        ) + _PINNED_NM_DATA + [_MAKE] + sorted(overlay.keys()),
        size = "small",
        tags = ["output"],
    )
...
def shell_exercise(
    ...
    Args:
      ...
      files: emit exNN_files, default True. Both shell subjects say on p.3 "You
        must not leave any additional files in your directory other than those
        specified in the assignment", and neither module checked it -- appending
        one `echo leftover > oops.txt` to a generator left every layer green,
        and //tools:submit pushes what is there with `git add -A --force`.

        It asserts the EXTRAS half only: `required` is empty and the
        deliverable is `optional`, with extras strict. A deliverable that was
        not produced at all is already the output layer's finding, stated
        precisely and with that exercise's clues attached, so requiring it here
        as well would print two reds for one cause -- and on the template
        branch, where every generator is a skeleton, it would print one per
        exercise for a state the module is supposed to be in.

        Pass False where the deliverable's NAME is itself the answer, which is
        the one case that cannot be written down here: shell-01 ex05's file is
        named with shell metacharacters and naming it in a BUILD file would
        hand over the exercise. Emitting the layer with no deliverable name is
        a fail(), because the alternative is a target that reports the
        student's own answer as an unasked-for extra.
    ...
    if files:
        if not deliverable:
            fail(("shell_exercise(num = \"%s\") emits a files layer but names no " +
                  "deliverable, so the layer would report the exercise's own " +
                  "answer as a file nobody asked for. Name it, or pass " +
                  "files = False and say why it cannot be named.") % num)

        # The generator runs with the scratch dir as cwd, exactly as it does in
        # deliverable/exNN under //tools:generate -- and that directory is what
        # //tools:submit pushes. So the names it leaves behind here ARE the
        # names 42 receives.
        f_args = [
            "--generator",
            "$(location %s)" % gen,
            ...
            "--label",
            "%s/deliverable/%s" % (native.package_name(), ex),
            "--optional",
            deliverable,
            "--strict",
        ]
...
def rush_variant(
    ...
    Args:
        ...
        gated: default True. False tags every emitted test "manual", which is
            how the four variants a team does NOT owe stay out of
            `bazel test //...` and out of the submit gate -- see the block
            comment above rush_common. A team owes exactly one variant; failing
            them on four unwritten bonuses would be a gate that blocks a push
            over work nobody asked for.
        exports: the global symbols rush0N.c may define, default ["rush"].
            The evaluator REPLACES main.c at the defense, so every non-static
    ...
    robust_expected = "%s/robust.txt" % tests
