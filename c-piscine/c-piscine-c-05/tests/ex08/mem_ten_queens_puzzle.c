 ...
 *
 *   - the board itself. Whatever storage represents it is sized by a count
 *     the implementation chose, and the classic slip is writing one element
 *     past its end — a loop bound or an initialisation that runs one step too
 *     far. That write lands one element past the end of a stack array.
 *   - any buffer the output is assembled in, if there is one. Sizing it for the
 *     digits and then writing the newline into it as well is an off-by-one that
 *     produces byte-perfect output, because the stray byte is overwritten by
 *     the next line before anyone reads it.
 *
