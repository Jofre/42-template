...
#     bazel test //c-piscine/c-piscine-c-00:basic
#
...
if [ -f .bazelrc.local ]; then
	if grep -q 'output_user_root' .bazelrc.local; then
		...
		add_local_line 'output_user_root' \
			"startup --output_user_root=$SCRATCH/bazel" "output_user_root"
	fi
...
EOF
	say ".bazelrc.local: written (output_user_root -> $SCRATCH/bazel, disk cache -> $SCRATCH/bazel-disk)"
...
_tc_fixed=0
for _cc in "$SCRATCH"/bazel/*/external/*local_config_cc/BUILD \
           "$SCRATCH"/bazel-gate/external/*local_config_cc/BUILD; do
	[ -f "$_cc" ] || continue
	_tc_bad=""
...
say "  bazel test //c-piscine/c-piscine-c-00:basic"
