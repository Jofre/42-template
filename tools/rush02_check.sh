...
while IFS='	' read -r f1 f2 f3; do
	...
	got=$("$BIN" "$use_dict" "$num" 2> /dev/null < /dev/null)
	st=$?
	...
	# the same hole the three siblings closed. Above 128 is a signal death: this
	# runner sets no ulimit and imposes no timeout, so nothing else can produce
	# one.
	if [ "$got" != "$want" ] || [ "$st" -ge 128 ]; then
		BAD=$((BAD + 1))
		if [ "$BAD" -le "$SHOW" ]; then
			if [ "$st" -ge 128 ]; then
				printf '  %-24.24s | %-34.34s | (killed by signal %d)\n' \
					"$num" "$want" "$((st - 128))"
			else
