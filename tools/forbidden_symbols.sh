...
# flags produces exactly ONE object referencing any of them,
# c-piscine/c-piscine-rush-01/deliverable/ex00/rush01-01.c with `U memcpy`, emitted for
# its permutations[24][4] initialiser and with no such call anywhere in the
# file.
#
...
#
# CALL-SHAPED, not a bare word match. A deliverable that merely names memcpy in
# a comment has not called it, and failing it for prose would be the false-red
# class this repo ranks alongside false greens. Written without \< or \b, which
# are GNU extensions this file cannot assume.
#
...
for b in $BASELINE; do
	# shellcheck disable=SC2086
	if grep -Eq "(^|[^A-Za-z0-9_])${b}[[:space:]]*\(" $SRCS 2> /dev/null; then
		continue
	fi
	EXEMPT="$EXEMPT $b"
done
...
for sym in $(sort -u "$UND"); do
	...
	ok=0
	for a in $ALLOWED_LIST $EXEMPT; do
		if [ "$sym" = "$a" ]; then
			ok=1
			break
		fi
