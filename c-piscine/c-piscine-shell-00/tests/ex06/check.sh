...
#
# Hardening note: the Moulinette compares stdout byte-for-byte. The listing is
# one path per line, sorted, each line terminated by a newline — the subject's
...
#   tracked.txt is staged (in the index) -> tracked, so it must NOT be listed
# Git lists paths in sorted order, so the expected listing is fully determined
# and host-independent (a global ignore file set in the host's git config can
# only change whether these files are ignored — they always are, via the local
# .gitignore).
...
# 2) A tracked file must NOT appear (it is in the index, so it is not ignored).
