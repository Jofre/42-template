 ...
 * there. It only breaks when the argument is an EXPRESSION. Take a made-up
 * macro that has nothing to do with this exercise, DOUBLE(x) defined as
 * `x * 2`: DOUBLE(1 + 1) expands to `1 + 1 * 2`, and operator precedence
 * quietly makes that 3, not 4. The same trap is waiting for EVEN(1 + 1), the
 * answer flips, and nothing in the subject's example would ever have shown
 * you.
 *
 ...
 *
 *     bazel test //c-piscine/c-piscine-c-08:basic    subject fidelity — this is not in it
 *     bazel test //c-piscine/c-piscine-c-08:strict   this too
 *
