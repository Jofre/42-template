 ...
 * cases pass an expression (2 - 5) as the argument; the "outer" cases embed
 * ABS(...) inside a larger expression (2 * ABS(-3)). Both only come out right
 * if the text the preprocessor pastes in still means what it says once C's
 * precedence rules are applied to it. */
int	main(void)
{
	printf("%s\t%d\n", "ABS(0)", ABS(0));
	printf("%s\t%d\n", "ABS(5)", ABS(5));
