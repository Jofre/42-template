...
|---|---|---|
| C Piscine | `c-piscine/` | `c-piscine-c-00` … `c-piscine-c-13`, the Shell Piscine (`c-piscine-shell-00`, `c-piscine-shell-01`), the three weekend **Rushes**, **BSQ** (the final project) and the final-exam guide |
...
```sh
bazel test //c-piscine/c-piscine-c-00:basic                             # the first module
bazel test //c-piscine/c-piscine-c-00:ex01_output --test_output=errors  # one exercise
```
...
```sh
bazel test //c-piscine/c-piscine-c-00:ex01_output --test_env=CLUE_MODE=all
```

### 4. Go green

Write the body in `c-piscine/c-piscine-c-00/deliverable/ex01/ft_print_alphabet.c` and run
that command again. Bazel prints a log only for failures, so a pass is one line:

```
//c-piscine/c-piscine-c-00:ex01_output                                   PASSED in 0.1s
```
...
bazel test //c-piscine/c-piscine-c-05:basic     # one module, what the Moulinette grades
...
| prepare for the final exam | [c-piscine/c-piscine-exam-prep/FINAL_EXAM_GUIDE.md](c-piscine/c-piscine-exam-prep/FINAL_EXAM_GUIDE.md), also as a PDF and a standalone page beside it (rebuilt from the Markdown with `sh tools/exam_guide.sh`) |
...
reads them, and `.gitignore` already ignores `*.pdf`, so a fork cannot republish
them by accident.
...
rush-02. Download your own from the intranet. shell-00 ex07, the one exercise that
needs that tarball, SKIPS with an explanation until you do.
