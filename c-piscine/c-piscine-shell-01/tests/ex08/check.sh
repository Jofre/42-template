...
# input, because Example 1's operands are small: their values fit in 64 bits,
# and at that size a program whose arithmetic silently overflows on bigger
# numbers lands on the SAME bytes as one that does not, digest and trailing
# newline included. Example 1 is therefore precisely the input at which the
# right and the wrong program coincide, and such a program scored a perfect 3/3
# here. Example 2's operands do not fit in 64 bits, so a program that overflows
# prints garbage there while a correct one does not. A worked example printed
# in a subject IS a test case; this one hands over the exact input that
# separates the two implementations, and the suite was throwing it away. (It
# costs a correct solution nothing: a working add_chelou.sh reproduces the
# subject's Example 2 result byte-for-byte.)
...
# quote, so each is a plain single-quoted literal here. Their values do not fit
# in 64 bits — see the header for why that matters.
raw2=$(mktemp)
