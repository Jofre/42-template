 ...
 * lists whose data is a malloc'd int*, merges the second into the first with
...
int	main(void)
{
	...
	while (dio_line(line, sizeof(line)))
	{
		if (dio_split(line, f, 3) < 2)
			continue ;
		begin1 = build_int(tab, parse_csv(f[0], tab));
		begin2 = build_int(tab, parse_csv(f[1], tab));
		ft_sorted_list_merge(&begin1, begin2, &cmp_int);
		printf("%s\t%s\t", f[0], f[1]);
		ser_int(begin1);
		free_int(begin1);
	}
