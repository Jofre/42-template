...
while IFS= read -r line; do
	...
	#
	# 128+N is "killed by signal N" from the shell, which is the only way a
	# crash reaches here -- the child is not run under a shell that reports
	# otherwise. tools/argv_table.sh:185 states this rule for its own layer:
	# "this is the one thing this layer must not miss".
	cmp -s "$WORK/want" "$WORK/got" && [ "$st" -lt 128 ] && continue
	FAILED=$((FAILED + 1))
