 ...
 * Line: <seq>\t<shape>\t<hex@level@first,...>. seq = the items, comma-joined
 * hex; shape = where each one hangs ("-" the root, "3L" left of item 3). Links
 * the tree from those two, reprints <seq>\t<shape>\t then drives the student's
 * btree_apply_by_level with a fixed applyf that emits, per visited node,
 * <hex>@<level>@<is_first>, comma-separated. See tools/diffio.h. */
#include "diffio.h"
#include "ft_btree.h"
