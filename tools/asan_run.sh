...
#
# When a student's Makefile builds nothing, make_srcs_build.sh --stub-on-failure
# writes a small /bin/sh script in the binary's place, so that every test fails
# one at a time with its own clues instead of the whole module failing to build.
# That script prints the build failure and exits 1 -- and exit 1 is below 128
# and names no sanitizer, so this runner used to fall through to "OK --
# memory-clean on this case (exit 1)". Found on rush-02's stub Makefile: nine
# ASan targets green for a program that did not exist, beside valgrind targets
# correctly saying SKIP. The output layer is already red for this case, and it
# is the one with the clues, so this steps aside the same way the gated layers
# do -- and NO_SKIP=1 turns the skip into the failure it would otherwise hide.
if head -c 256 "$BIN" 2> /dev/null | grep -q 'Written by tools/make_srcs_build.sh: there was no program to build'; then
	[ "${NO_SKIP:-0}" != "1" ] || {
		echo "NO_SKIP set: ${LABEL:-the program} was never built -- the Makefile produced"
		echo "             nothing -- so there was no program to check for memory safety."
		exit 1
	}
	echo "asan_run: SKIP — ${LABEL:-the program} was never built: the Makefile produced no"
	echo "          program, so there is nothing here to check for memory safety. The"
	echo "          output layer for this case says why, with its clues."
	exit 0
fi
