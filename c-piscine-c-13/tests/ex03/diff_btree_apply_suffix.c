/* Live-differential reader harness for btree_apply_suffix.
 * Line: <seq>\t<suffix-hex-csv>. Builds the tree with a local strcmp BST
 * insert (lower left, equal-or-higher right), reprints <seq>\t then drives the
 * student's btree_apply_suffix with a fixed applyf that emits each item's
...
int	main(void)
{
	...
	while (dio_line(line, sizeof(line)))
	{
		...
		g_first = 1;
		btree_apply_suffix(root, apply_print);
		putchar('\n');
		free_tree(root);
		i = 0;
		while (i < n)
