...
require awk basename cut dirname grep head mktemp readlink rm sed
# conventions: optional timeout -- without it an unverified program is bounded
# only by Bazel's own test timeout: the target still reds, but the message says
# nothing a student can act on. It ships with the same coreutils as head and wc,
# so the branch is unreachable wherever the require list above passes.
...
# in its own comments, and it would be printed on every green.
if [ "$NO_LEAK_CHECK" = 1 ]; then
	CLEAN_CLAIM="no invalid access, no uninitialised value (leaks not checked)"
else
	CLEAN_CLAIM="no leak, no invalid access, no uninitialised value"
fi
...
report_fds() {
	...
	grep -A2 'Open file descriptor' "$VGLOG" 2>/dev/null |
		grep -v '<inherited from parent>' | sed -n '1,6p' | sed 's/^/    /'
	echo ""
	echo "  Every open() hands back a number the kernel is holding for you until"
	echo "  you give it back or the process ends. A program that ends promptly"
	echo "  gets away with it; one that opens a file per argument does not, and"
	echo "  the limit it eventually meets is a few hundred, not a few million."
	echo ""
	if [ "${VALGRIND_FDS:-}" = "lax" ]; then
		echo "  VALGRIND_FDS=lax, so this is a NOTE rather than a failure."
	else
		echo "  This is a FAILURE. Every descriptor open() hands back is one the"
		echo "  kernel holds until you close it or the process dies; a program"
		echo "  that opens one per argument meets the limit for real. Set"
		echo "  VALGRIND_FDS=lax if you need to get past it for now."
	fi
}
...
# The descriptor note rides on the SUCCESS path as well as the failure ones:
# leaving a file open is orthogonal to every memory finding, so a run that is
# clean on memory is exactly where it would otherwise never be mentioned.
if [ "$RC" -eq 0 ]; then
	echo "valgrind_test: OK — $CLEAN_CLAIM ($LABEL)"
	[ -n "$HEAP" ] && echo "               ${HEAP}"
	report_fds
	if [ "${VALGRIND_FDS:-}" != "lax" ] && fds_left_open; then
		exit 1
	fi
	exit 0
fi
...
report_fds
# The SAME condition as the clean-exit path above, and it has to be: report_fds
# has just printed "This is a FAILURE" unless VALGRIND_FDS=lax, so any other
# test here makes the layer contradict itself in the same breath.
#
# It used to read = "strict", a value nothing in this tree ever sets -- not
# .bazelrc, not defs.bzl, not a runner, not a doc -- so on this path the gate
# was dead: the report said FAILURE and the layer exited 0. Reproduced with two
# programs differing only in main's return value, one leaving a descriptor open
# under each. A descriptor left open is not less of a defect because the program
# also chose a non-zero exit status; the exit status is the *_output layer's
# business, which the message a few lines up says outright.
#
# Costs nothing today: all 69 valgrind targets pass, and no test log in the tree
# contains "the program chose to exit" -- the same measurement that was made
# before gating the clean-exit path.
if [ "${VALGRIND_FDS:-}" != "lax" ] && fds_left_open; then
	exit 1
fi
exit 0
