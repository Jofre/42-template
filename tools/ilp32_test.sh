...
require dirname id mkdir mktemp rm sed
...
if [ -n "$ZIG" ] && [ -x "$ZIG" ]; then
	...
	# person to sit down. That is the identical hazard .bazelrc documents at
	# length for /tmp/bazelcache, which is why the name carries $USER there too.
