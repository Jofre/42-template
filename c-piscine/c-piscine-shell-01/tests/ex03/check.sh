...
#
# Trailing-newline note: the output is a single newline-terminated integer, as
# the subject's cat -e transcript shows (a '$' right after the number). The
# Moulinette compares stdout byte-for-byte, so a solution that strips that final
# newline (or adds a second line / stray spaces) is a real KO. The previous
# check squashed all whitespace with `tr -d '[:space:]'` and was blind to every
# one of those. We now capture the RAW bytes and assert the exact terminator +
# single-line integer format.
...
# 2) Output ends with a newline, as the subject's cat -e shows; a stripped final
...
#    spaces and no extra text -- the subject's cat -e shows nothing but the
#    number before its '$'.
